...
function sanitize(url: URL): {
  ...
  // Recovery links land on the password form; everything else finishes
  // signup. The template passes next explicitly; this is the fallback.
  const fallback =
    rawType === "recovery" ? "/staff/reset-password" : "/staff/sign-up";
...
export async function GET(request: Request) {
  ...
  // Copy matches what the click actually does: recovery links lead to
  // the password form; everything else confirms the email address.
  const copy =
    type === "recovery"
      ? {
          title: "Reset your password — MyTableView",
          heading: "Almost there",
          body: "Press the button to continue to the page where you choose a new password.",
          button: "Choose a new password",
        }
      : {
          title: "Confirm your email — MyTableView",
          heading: "One last click",
          body: "Press the button to confirm your email address and continue setting up your restaurant.",
...
<body style="margin:0;min-height:100dvh;display:grid;place-items:center;background:#f6f4ef;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#16293d;">
  <main style="width:100%;max-width:22rem;padding:2rem 1.75rem;background:#ffffff;border:1px solid #e0dace;border-radius:1rem;box-shadow:0 18px 40px rgba(22,41,61,0.08);text-align:center;">
    ...
    <h1 style="margin:0 0 0.6rem;font-size:1.0625rem;font-weight:600;color:#5c6b7a;">${copy.heading}</h1>
    <p style="margin:0 0 1.5rem;font-size:0.9375rem;line-height:1.55;color:#5c6b7a;">${copy.body}</p>
    <form method="post" action="/auth/confirm">
      <input type="hidden" name="token_hash" value="${tokenHash}">
      <input type="hidden" name="type" value="${type}">
      <input type="hidden" name="next" value="${next.replace(/"/g, "")}">
      <button type="submit" style="width:100%;padding:0.8125rem;font-size:1rem;font-weight:650;color:#ffffff;background:#0b5f56;border:none;border-radius:0.5rem;cursor:pointer;">${copy.button}</button>
    </form>
  </main>
</body>
</html>`;
